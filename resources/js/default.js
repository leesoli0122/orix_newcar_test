...
function openPopup(id) {
    ...
    // 화면 크기가 1200px 이상인지 확인
    const isPcScreen = window.matchMedia("(min-width: 1201px)").matches;

    // PC 화면에서 estimate가 포함된 팝업이 열리지 않도록 처리
    if (isPcScreen && isEstimate) {
        console.log("PC 화면에서는 estimate 팝업이 열리지 않습니다.");
        return;
    }
